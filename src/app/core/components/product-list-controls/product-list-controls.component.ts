...
export class ProductListControlsComponent implements OnChanges {
    ...
    // Added by me to alter filters behaviour to AND or OR
    @Output() andFiltersChanged = new EventEmitter<boolean>();
    andFilters = false;

    toggleAndFilters() {
        this.andFilters = !this.andFilters;
        this.andFiltersChanged.emit(this.andFilters);
        this.activeFacetValueIds = [];
        this.toggleFacetValueIdInRoute('');
    }
